...
class BooksSpider(scrapy.Spider):
    ...
    # item = BooksScraperItem()    -> if we initialize item here, it will be overwritten in each iteration
    
    def parse(self, response):
        for book in response.css("article.product_pod"):
            item = BooksScraperItem()  # item = BooksScraperItem()   -> if we initialize item here, it will fresh new object in each iteration
